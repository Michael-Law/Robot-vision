...
img1 = cv2.imread("./data_2/0000000000.png", 0)  # queryimage # left image
img2 = cv2.imread("./data_2/0000000001.png", 0)  # trainimage # right image

crop_size = 1800
image1_cp = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)[:crop_size, :crop_size]
image2_cp = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)[:crop_size, :crop_size]
...
P_real = (
    np.real(f1_shf_cplx) * np.real(f2_shf_cplx)
    + np.imag(f1_shf_cplx) * np.imag(f2_shf_cplx)
) / total_abs
P_imag = (
    np.imag(f1_shf_cplx) * np.real(f2_shf_cplx)
...
cv2.rectangle(
    image1_rgb,
    (0, 0),
    (image1_rgb.shape[0] - shift_x, image1_rgb.shape[1] - shift_y),
    (0, 255, 0),
    10,
)
cv2.rectangle(
    image2_rgb,
    (shift_x, shift_y),
    (image2_rgb.shape[0], image2_rgb.shape[1]),
    (0, 255, 0),
    10,
)
